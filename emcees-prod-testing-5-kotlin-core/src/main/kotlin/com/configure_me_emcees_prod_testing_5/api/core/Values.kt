...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw EmceesProdTesting5InvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw EmceesProdTesting5InvalidDataException("`$name` is not set")
            is JsonNull -> throw EmceesProdTesting5InvalidDataException("`$name` is null")
            else ->
                throw EmceesProdTesting5InvalidDataException("`$name` is invalid, received $this")
        }
