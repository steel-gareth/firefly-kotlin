...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw FireflyInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw FireflyInvalidDataException("`$name` is not set")
            is JsonNull -> throw FireflyInvalidDataException("`$name` is null")
            else -> throw FireflyInvalidDataException("`$name` is invalid, received $this")
        }
