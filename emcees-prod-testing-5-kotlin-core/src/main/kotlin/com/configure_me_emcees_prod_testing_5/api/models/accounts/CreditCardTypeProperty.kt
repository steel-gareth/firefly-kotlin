...
private constructor(private val value: JsonField<String>) : Enum {
    ...
    companion object {
        ...
        val MONTHLY_FULL = of("monthlyFull")

        fun of(value: String) = CreditCardTypeProperty(JsonField.of(value))
    }

    /** An enum containing [CreditCardTypeProperty]'s known values. */
    enum class Known {
        MONTHLY_FULL
    }

    /**
     * An enum containing [CreditCardTypeProperty]'s known values, as well as an [_UNKNOWN] member.
     *
     * An instance of [CreditCardTypeProperty] can contain an unknown value in a couple of cases:
    ...
    enum class Value {
        MONTHLY_FULL,
        /**
         * An enum member indicating that [CreditCardTypeProperty] was instantiated with an unknown
         * value.
         */
        _UNKNOWN,
    }
    ...
    fun value(): Value =
        when (this) {
            MONTHLY_FULL -> Value.MONTHLY_FULL
            else -> Value._UNKNOWN
        }
    ...
    fun known(): Known =
        when (this) {
            MONTHLY_FULL -> Known.MONTHLY_FULL
            else ->
                throw EmceesProdTesting5InvalidDataException(
                    "Unknown CreditCardTypeProperty: $value"
                )
        }
     ...
     *   its expected type.
     */
    fun validate(): CreditCardTypeProperty = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is CreditCardTypeProperty && value == other.value
    }
