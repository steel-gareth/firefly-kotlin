...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw MoreConflictingInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MoreConflictingInvalidDataException("`$name` is not set")
            is JsonNull -> throw MoreConflictingInvalidDataException("`$name` is null")
            else -> throw MoreConflictingInvalidDataException("`$name` is invalid, received $this")
        }
